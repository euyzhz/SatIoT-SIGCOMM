...
DEVICE_CONFIGS = [
    {"logical_id": 1, "hw_id": 12185, "port": "COM5"},
    {"logical_id": 2, "hw_id": 12174, "port": "COM6"},
    {"logical_id": 3, "hw_id": 12171, "port": "COM9"},
]
...
# 图片发送完后，每小时轮询一次三个设备的 CMMQ
RESTART_POLL_INTERVAL = 3600.0

# 是否保存编码后的中间结果，便于检查
SAVE_DEBUG_IMAGES = True
DEBUG_OUTPUT_DIR = Path("./debug_output_3dev")

# 完成记录目录与总日志文件
IMAGE_FINISH_RECORD_DIR = Path(".")
IMAGE_FINISH_LOG_FILE = IMAGE_FINISH_RECORD_DIR / "image_finish_log_3dev.txt"

# 串口日志总目录
DEVICE_LOG_ROOT_DIR = Path("./log_3dev")
...
def build_l0_payloads(l0_arr: np.ndarray) -> List[bytes]:
    ...
    chunks = [
        raw[i:i + MAX_PAYLOAD_BYTES]
        for i in range(0, len(raw), MAX_PAYLOAD_BYTES)
    ]
...
def encode_image_for_devices(image_file: str) -> Dict[int, List[EncodedFrame]]:
    ...
    l0_payloads = build_l0_payloads(l0_arr)
    print(f"{get_timestamp()} L0: {L0_WIDTH}x{L0_HEIGHT}, 共 {len(l0_payloads)} 帧")
    ...
    if total_tiles != expected_tiles:
        raise RuntimeError(
            f"EL tile 数异常: 实际 {total_tiles}, 预期 {expected_tiles}"
        )

    save_debug_artifacts(img, l0_arr, g_target_arr, el_tiles_sorted)

    device_ids = [cfg["logical_id"] for cfg in DEVICE_CONFIGS]
    el_alloc = distribute_el_tiles_round_robin(el_tiles_sorted, device_ids)

    out: Dict[int, List[EncodedFrame]] = {did: [] for did in device_ids}

    for did in device_ids:
        total_l0 = len(l0_payloads)
        for i, payload in enumerate(l0_payloads):
            out[did].append(
                EncodedFrame(
                    layer_id=LAYER_L0,
                    fragment_index=i,
...
def start_new_image_cycle():
    ...
    print()
    print("====================================================")
    print(f"{get_timestamp()} 启动新一轮图片发送（3设备共发版）")
...
def finish_current_image_cycle():
    ...
    if not CURRENT_CYCLE_FINISH_RECORDED:
        print(f"{get_timestamp()} 所有设备帧已发送完成")
...
def try_restart_when_all_queues_empty():
    """
    3设备共发版规则：
    三个设备里，只要有任意一个还有待发数据，就不启动下一轮；
    必须三个都为 0，才开始下一轮。
    """
    print(f"{get_timestamp()} 开始执行三设备队列检查（AT+CMMQ?）...")
    ...
    if all(count == 0 for count in queue_counts):
        print(f"{get_timestamp()} 三个设备都没有待发数据，启动下一轮图片发送")
...
def main_loop():
    ...
    print(f"{get_timestamp()} 进入图片循环发送模式（3设备共发版）")
    print(f"{get_timestamp()} 每个设备独立发送间隔: {SEND_INTERVAL} 秒")
    print(f"{get_timestamp()} 图片发送完毕后，每 {int(RESTART_POLL_INTERVAL)} 秒检查一次全部3个设备的 CMMQ")
    print()
...
def init_system():
    ...
    print("LoRa 3设备共发图片循环发送程序")
    print("====================================")
    print(f"图片文件: {IMAGE_FILE}")
    print(f"基础层 L0: {L0_WIDTH}x{L0_HEIGHT} 灰度，三台设备重复发送")
    print(
        f"独立图块层 EL: {EL_WIDTH}x{EL_HEIGHT}, "
        f"tile={TILE_WIDTH}x{TILE_HEIGHT}，三台设备分摊"
    )
    print(f"帧长度: {FRAME_SIZE_BYTES} 字节（头 {HEADER_SIZE_BYTES} + 载荷最多 {MAX_PAYLOAD_BYTES}）")
    print("初始化 AT 命令无响应自动重发")
    print("AT+CMMQ? 无响应自动重发")
    print("AT+SEND 不自动重发，避免重复入队")
    print("图片发送完毕后，检查3设备 CMMQ?；只要有一个非0，就不开始下一轮")
    ...
    for cfg in DEVICE_CONFIGS:
        ...
        print(
            f"{get_timestamp()} [DEV{dev.logical_id}] "
            f"单轮待发送: 总计 {len(dev.frames)} 帧, L0={l0_count}, EL={el_count}"
        )
...
def cleanup():
    ...
    for dev in DEVICE_STATES:
        ...
        fail = total - succ
        print(
            f"{get_timestamp()} [DEV{dev.logical_id}] 发送统计 - 总计:{total} 成功:{succ} 失败:{fail}"
        )
