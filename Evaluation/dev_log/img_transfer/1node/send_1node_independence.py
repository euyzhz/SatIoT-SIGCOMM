...
# 本程序实际运行的设备（1台）
DEVICE_CONFIGS = [
    {"logical_id": 1, "hw_id": 12174, "port": "COM6"},
]
...
DEBUG_OUTPUT_DIR = Path("./debug_output_1dev")

IMAGE_FINISH_RECORD_DIR = Path(".")
IMAGE_FINISH_LOG_FILE = IMAGE_FINISH_RECORD_DIR / "image_finish_log_1dev.txt"

DEVICE_LOG_ROOT_DIR = Path("./log_1dev")
...
def encode_image_for_devices(image_file: str) -> Dict[int, List[EncodedFrame]]:
    ...
    print(f"{get_timestamp()} 单设备版 L0扩展后: 共 {len(l0_payloads_full)} 帧")
    ...
    if len(running_ids) != 1:
        raise RuntimeError("单设备版 DEVICE_CONFIGS 必须只有 1 台设备")

    did = running_ids[0]
    out: Dict[int, List[EncodedFrame]] = {did: []}

    # 48 个 L0 全部给这一台
    total_l0 = len(l0_payloads_full)
    ...
    # 192 个 EL 全部给这一台
    total_el = len(el_tiles_sorted)
    print(f"{get_timestamp()} DEV{did} 分配到 EL tile 数: {total_el}")

    for i, (tile_id, tile_payload, _score) in enumerate(el_tiles_sorted):
...
def start_new_image_cycle():
    ...
    print()
    print("====================================================")
    print(f"{get_timestamp()} 启动新一轮图片发送（单设备版）")
...
def finish_current_image_cycle():
    ...
    if not CURRENT_CYCLE_FINISH_RECORDED:
        print(f"{get_timestamp()} 当前设备帧已发送完成")
...
def try_restart_when_all_queues_empty():
    ...
    它自己为 0，才开始下一轮。
    """
    print(f"{get_timestamp()} 开始执行单设备队列检查（AT+CMMQ?）...")
    ...
    if all(count == 0 for count in queue_counts):
        print(f"{get_timestamp()} 当前设备没有待发数据，启动下一轮图片发送")
        start_new_image_cycle()
    else:
        print(f"{get_timestamp()} 当前设备仍有待发数据，暂不启动下一轮")
...
def main_loop():
    ...
    print(f"{get_timestamp()} 进入图片循环发送模式（单设备版）")
    print(f"{get_timestamp()} 每个设备独立发送间隔: {SEND_INTERVAL} 秒")
    print(f"{get_timestamp()} 图片发送完毕后，每 {int(RESTART_POLL_INTERVAL)} 秒检查一次当前设备的 CMMQ")
    print()
...
def init_system():
    ...
    print("LoRa 单设备图片循环发送程序")
    print("====================================")
    print(f"图片文件: {IMAGE_FILE}")
    print("EL 分片仍按 3 设备逻辑分成三份，本程序只发送当前这一台设备自己的那一份")
    print(f"帧长度: {FRAME_SIZE_BYTES} 字节（头 {HEADER_SIZE_BYTES} + 载荷最多 {MAX_PAYLOAD_BYTES}）")
    print("图片发送完毕后，只检查当前设备的 CMMQ?；为 0 才开始下一轮")
